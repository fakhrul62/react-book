...
export async function getHomeData() {
  ...
  const [subjectLinksResult, featuredResult, recentResult] = await Promise.all([
    supabase.from("book_subjects").select("subject_id, subjects(id, name, slug)").limit(1200),
  ...
  for (const link of subjectLinksResult.data || []) {
    const subject = link.subjects;
    if (!subject) continue;
    const current = subjectMap.get(subject.id) || { ...subject, book_count: 0 };
    current.book_count += 1;
    subjectMap.set(subject.id, current);
  }
  const subjects = [...subjectMap.values()]
    .filter((subject) => subject.book_count > 0)
  ...
  return {
    ...
    error: subjectLinksResult.error || featuredResult.error || recentResult.error || null
  };
}
