  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-ink/10">
        <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8">
          <div className="max-w-3xl">
            <p className="text-sm font-bold uppercase tracking-[0.28em] text-plum">Open Library, quietly curated</p>
            <h1 className="mt-5 font-display text-6xl font-bold leading-[0.9] text-ink sm:text-8xl lg:text-9xl">Find the book that stays with you.</h1>
          ...
          <div className="relative mx-auto w-full max-w-lg">
            <div className="absolute inset-8 rotate-6 rounded-lg bg-plum/15" />
            <div className="absolute inset-4 -rotate-3 rounded-lg bg-gilt/20" />
            {heroBook ? (
              ...
              <div className="relative rounded-lg border border-ink/10 bg-white/70 p-10 shadow-book">
                <p className="font-display text-4xl font-semibold">Your synced shelf will appear here.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-6">
          <div>
            ...
            <h2 className="mt-2 font-display text-5xl font-bold text-ink">Living categories</h2>
          </div>
        </div>
        {subjects.length ? (
          <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {subjects.map((subject) => (
              <Link key={subject.id} href={`/category/${subject.slug}`} className="focus-ring cursor-pointer rounded-lg border border-ink/10 bg-white/65 p-5 transition-colors duration-200 hover:border-moss hover:bg-white">
                <span className="block font-display text-3xl font-semibold leading-tight text-ink">{subject.name}</span>
                <span className="mt-3 block text-xs font-bold uppercase tracking-[0.18em] text-ink/50">{subject.book_count || 0} books</span>
              </Link>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between">
            <div>
              ...
              <h2 className="mt-2 font-display text-5xl font-bold">Featured books</h2>
            </div>
          </div>
          {featured.length ? (
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <h2 className="mt-2 font-display text-5xl font-bold text-ink">Recently added</h2>
        {recent.length ? (
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
